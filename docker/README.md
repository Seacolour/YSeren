...
目前尚未向 Docker Hub、GHCR 或其他镜像仓库发布镜像。这里的命令只在
本机 Docker Desktop 中构建 `yseren:local`。
...
2. 在 CI 中用 Buildx 构建 `linux/amd64`、`linux/arm64`，运行 amd64 smoke。
3. 确认镜像标签策略，例如版本 tag、次版本 tag 和 `latest`。
4. 发布到目标仓库后记录不可变 digest，并按需生成 SBOM/签名。
5. 在至少一种真实 NAS/arm64 设备上验证目录权限、性能和持续运行。
